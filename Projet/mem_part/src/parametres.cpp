...
//_ ______________________________________________________________________________________________ _
bool apparition_technologie(const parametres &p, std::uniform_real_distribution<double> &rd,
                            std::minstd_rand0 &generator)
{
    // double val = std::rand()/(1.*RAND_MAX);
    double val = rd(generator);
...
bool a_un_systeme_proche_colonisable(int i, int j, int width, int height, const char *galaxie)
{
    ...
    if ((i > 0) && (galaxie[(i - 1) * width + j] == habitable))
        return true;
    if ((i < height - 1) && (galaxie[(i + 1) * width + j] == habitable))
        return true;
    if ((j > 0) && (galaxie[i * width + j - 1] == habitable))
        return true;
    if ((j < width - 1) && (galaxie[i * width + j + 1] == habitable))
        return true;
...
#pragma omp parallel for private(i, j) shared(galaxie_previous, galaxie_next) schedule(dynamic) num_threads(THREADS)
    for (i = 0; i < height; ++i)
    {
        for (j = 0; j < width; ++j)
        {
            unsigned seed = std::chrono::system_clock::now().time_since_epoch().count() + 84152 * omp_get_thread_num();
