...
namespace Bitonic
{
  ...
  // --------------------------------------------
  template<typename Obj> std::pair<Obj*,int>
  _sort( bool up, Obj* objs, int len, int depth )
  {
    if (len <= 1) return std::make_pair(objs,1);
    auto first  = _sort(true , objs, len/2, depth+1);
    auto second = _sort(false, objs + len/2, len - (len/2), depth+1);
    return _merge(up, first.first, first.second + second.second );
  }
  // --------------------------------------------
  template<typename Obj> void
  thread_sort(bool up, Obj* objs, int len, int depth, std::pair<Obj*,int>& result)
  {
    constexpr const int maxdepth = 0;
    if (depth < maxdepth) // Maximum 2^maxdepth
    {
      std::pair<Obj*,int> first, second;
      std::thread t1(thread_sort<Obj>, true , objs        , len/2, depth+1, std::ref(first));
      std::thread t2(thread_sort<Obj>, false, objs + len/2, len - len/2, depth+1, std::ref(second));
      t1.join();
      t2.join();
      result = _merge(up, first.first, first.second + second.second );
    }
    else
    {
      result = _sort(up, objs, len, depth);
    }
  }
  // --------------------------------------------
  ...
  {
    std::pair<Obj*,int> result;

    thread_sort(up, x.data(), int(x.size()), 0, result);
    //_sort(up, x.data(), int(x.size()), 0);
    return x;
  }  
}
