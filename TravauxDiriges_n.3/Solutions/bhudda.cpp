                            ...
                            std::vector<unsigned>& image )
{
     ...
     while ( (z2.re+z2.im < float(4)) && (i<maxIter) )
     {
        ...
        if ( (ip<width) && (jp<height) ) {
           int ind = ip+jp*width;
           // Il est important de mettre un atomic ici car il est toujours possible
           // que deux threads essayent simultanément d'incrémenter le même pixel.
#          pragma omp atomic           
           image[ind] += 1;
        }
        i ++;
...
#   pragma omp parallel
    {
        // Chaque thread va créer son propre générateur aléatoire.
        ...
        // Il est important ici de mettre la clause dynamique, car
        // chaque orbite calculée à un coût différent et faire une
        // parallélisation statique mènera à un mauvais équilibre
        // des tâches. On est également obligé de changer la boucle
        // car OpenMP ne peut pas paralléliser une boucle où l'incrément
        // n'est pas fixe et explicite dans l'instruction for.
        // NB : On a mis des paquets de 10 pour que les calculs par tâches
        // soient conséquentes (granularité de tâche).
#   pragma omp for schedule(dynamic,10)
        for ( unsigned long iSample = 0; iSample < nbSamples; ++iSample) {
            ...
            do
            {
                ...
                is_divergent = test_mandelbrot_divergent( maxIter, c0 );
                if (is_divergent)
                {
